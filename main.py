...
import time
username = "skkrt"
scope = "user-top-read, user-modify-playback-state, playlist-modify-public, playlist-modify-private, playlist-read-private"
...
############################################################
#create a playlist based off of top artists, genres, and genres

#define this as a function so that it can be called multiple times
def createPlaylist():
...
#########################################################################
#   FIND USER PLAYLISTS AND DOWNLOAD THEM TO THE COMPUTER AS MP3 FILES  #
#########################################################################
#define this as a function so that it can be called multiple times
def downloadPlaylists():
    #find the users playlists
    playlists = sp.current_user_playlists()
    #list the playlists names and assign them a number
    for i, playlist in enumerate(playlists['items']):
        print(i + 1, playlist['name'])
    #ask user which playlist they want to download
    playlistNum = int(input("Enter the number of the playlist you want to download: "))
    #add check so that the input is valid
    while playlistNum < 1 or playlistNum > len(playlists['items']):
        playlistNum = int(input("Invalid input. Enter the number of the playlist you want to download: "))
    #take the name of the playlist and put it into a variable
    playlistName = playlists['items'][playlistNum - 1]['name']
    #find the playlist id of the playlist
